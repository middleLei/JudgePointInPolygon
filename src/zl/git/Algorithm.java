...
/**
 * 判断一个点是否在多边形内的算法类，判断的 核心规则是以这个点做一个向右的水平射线
 * 
 * @author zl
 * 
 */
public class Algorithm {
	...
	/**
	 * 当射线与多边形的边相交的数量为奇数时，则相交，否则不相交。 <br />
	 * 算法的详细说明和证明这里不细说，这里主要考虑边界情况；<br />
	 * 第一：多边形顶点存在连续重复的点，比如点序列为p1，p2相等；<br />
	 * 第二：这个要判断的点本身就是多边形的某一个顶点；<br />
	 * 第三：这个要判断的点点不是顶点，通过射线法则计算相交的数量（包含在边上的点也采用这种算法，具体考虑见判断相交的算法）；<br />
	 * 
	 * @param point
	 *            要判断的点
	 * @param polygon
	 *            多边形顶点坐标
	 * @return 要判断的点在多边形内包括在边上返回true，否则返回false
	 */
	 ...
	 * 点p向右水平射线与多边形边p1、p2是否相交的判断逻辑：<br />
	 * 前提逻辑：如果点p1、p2的y值都大于或者小于p的y值，直接返回false（在一边不可能相交）；<br />
	 * 第一：通过计算出p0、p1的方程，计算出当y值于点p的y值相等时候的x值，如果x>p.x，则相交<br />
